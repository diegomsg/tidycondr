...
#' @param big_mark,decimal_mark Character for correct evaluation of numeric
#'  info stored as character.
#' @param currency_marks,ignore_before,ignore_after Characters to be ignore
#'  while evaluating.
#'
...
is_numeric_like <- function(x, big_mark = "",
                            decimal_mark = getOption("OutDec"),
                            currency_marks = "R$", ignore_before = "(",
                            ignore_after = "(") {
  ...
  if (!all(is.na(x))) {
    ...
    if (!missing(ignore_before)) {
      x <- gsub(ignore_before, "", x, fixed = TRUE)
    }
    if (!missing(ignore_after)) {
      x <- gsub(ignore_after, "", x, fixed = TRUE)
    }
    if (!missing(ignore_before) | !missing(ignore_after)) {
      x <- gsub("\\s+", "", x)
    }
    # replace big and decimal marks to international
    if (!missing(big_mark)) {
      x <- gsub(big_mark, "", x, fixed = TRUE)
    }
